...
void UTMXYToLatLon (FLOAT x, FLOAT y, int zone, bool southhemi, FLOAT& lat, FLOAT& lon) {
  ...
  //Transfer the lat & lon to angular degrees
  lat = RadToDeg(lat);
  lon = RadToDeg(lon);
...
// Returns:
// The function does not return a value.
void Resolution(coordinate& resolution, coordinate imgRowCol, coordinate UTMLeftUpper, coordinate UTMRightBottum)
{
  resolution.x = (UTMRightBottum.x - UTMLeftUpper.x) / imgRowCol.x;
  resolution.y = (UTMLeftUpper.y - UTMRightBottum.y) / imgRowCol.y;
}
...
{
  imgUV.x = floor((UTMxy.x - UTMLeftUpper.x)/resolution.x);
  imgUV.y = floor((UTMLeftUpper.y - UTMxy.y)/resolution.y);
}
...
{
  UTMxy.x = UTMLeftUpper.x + imgUV.x * resolution.x;
  UTMxy.y = UTMLeftUpper.y - imgUV.y * resolution.y;
}
...
{
  coordinate imgSize = {img.cols, img.rows};
  coordinate resolution = {0.0, 0.0};
  coordinate UTMLeftUpper = {0.0, 0.0};
  coordinate UTMRightBottom = {0.0, 0.0};
  coordinate UTMTemp = {0.0, 0.0};
  coordinate imgUV = {0.0, 0.0};

  LatLonToUTMXY(LatLonLeftUpper.y, LatLonLeftUpper.x, zone, UTMLeftUpper.x, UTMLeftUpper.y);
  LatLonToUTMXY(LatLonRightBottom.y, LatLonRightBottom.x, zone, UTMRightBottom.x, UTMRightBottom.y);
  Resolution(resolution, imgSize, UTMLeftUpper, UTMRightBottom);

  LatLonToUTMXY(LatLon.x, LatLon.y, zone, UTMTemp.x, UTMTemp.y);
  UTMxyToImgUV(resolution, UTMLeftUpper, UTMTemp, imgUV);
...
coordinate ImgUVToLatLon(cv::Mat img, coordinate LatLonLeftUpper, coordinate LatLonRightBottom, int zone, bool isSouth, coordinate imgUV)
{
  ...
  ImgUVToUTMxy(resolution, UTMLeftUpper, imgUV, UTMTemp);
  UTMXYToLatLon(UTMTemp.x, UTMTemp.y, zone, isSouth, lat, lon);
  LatLon = {lat, lon};
